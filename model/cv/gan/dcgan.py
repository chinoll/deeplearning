...
class Generator(nn.Module):
    def __init__(self,latent_dim,channels,img_size,**kwargs):
...
class Discriminator(nn.Module):
    def __init__(self,channels,img_size,**kwargs):
...
def model_init(latent_dim,channels,img_size,lr,b1,b2,weights_init,**kwargs):
...
def train(epoch,dataloader,generator,discriminator,g_optimizer,d_optimizer,adv_loss,latent_dim,vis,**kwargs):
    ...
    vis.line(X=torch.FloatTensor([epoch]), Y=torch.FloatTensor([np.mean(gloss)]), win='g loss', update='append' if i> 0 else None)
    vis.line(X=torch.FloatTensor([epoch]), Y=torch.FloatTensor([np.mean(dloss)]), win='d loss', update='append' if i> 0 else None)
